package androidx.lifecycle;

import java.util.HashMap;
import p000.EnumC1769;
import p000.InterfaceC1442;
import p000.InterfaceC1774;
import p000.InterfaceC1777;

/* JADX INFO: compiled from: obf */
/* JADX INFO: loaded from: classes.dex */
public final class CompositeGeneratedAdaptersObserver implements InterfaceC1774 {

    public final InterfaceC1442[] f291;

    public CompositeGeneratedAdaptersObserver(InterfaceC1442[] interfaceC1442Arr) {
        this.f291 = interfaceC1442Arr;
    }

    @Override // p000.InterfaceC1774
    public final void mo0(InterfaceC1777 interfaceC1777, EnumC1769 enumC1769) {
        new HashMap();
        InterfaceC1442[] interfaceC1442Arr = this.f291;
        if (interfaceC1442Arr.length > 0) {
            InterfaceC1442 interfaceC1442 = interfaceC1442Arr[0];
            throw null;
        }
        if (true) {
            return;
        }
        InterfaceC1442 interfaceC1443 = interfaceC1442Arr[0];
        throw null;
    }
}
