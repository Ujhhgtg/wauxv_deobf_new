package p000;

import bsh.AbstractC0016;
import bsh.Primitive;
import java.lang.reflect.Array;
import java.lang.reflect.Constructor;
import java.lang.reflect.Field;
import java.lang.reflect.Method;
import java.lang.reflect.Modifier;
import java.util.Arrays;
import java.util.HashSet;
import java.util.Iterator;

/* JADX INFO: renamed from: ᛱᛲᛳᛴᛵᛶᛷᤞᲁᲀᤝᲇᛸᲈ, reason: contains not printable characters */
/* JADX INFO: compiled from: obf */
/* JADX INFO: loaded from: classes.dex */
public final class C1910 {

    /* JADX INFO: renamed from: ᛱᛲᛳᛴᛵᛶᛷᛸᤝᤞᲀᲁᲇᲈ, reason: contains not printable characters */
    public final HashSet f6304;

    public C1910() {
        HashSet hashSet = new HashSet();
        this.f6304 = hashSet;
        hashSet.add(new C1909());
    }

    /* JADX INFO: renamed from: ᛱᛲᛳᛴᛵᛶᛷᛸᤝᤞᲀᲁᲇᲈ, reason: contains not printable characters */
    public final void m3841(Class cls, Object[] objArr) throws C2705 {
        Object[] objArrUnwrap = Primitive.unwrap(objArr);
        Iterator it = this.f6304.iterator();
        while (it.hasNext()) {
            if (!((InterfaceC2706) it.next()).mo3837(cls)) {
                int i = C2705.f8703;
                throw new C2705("Can't call this construct: new " + cls.getName() + "(" + C2705.m4674(objArrUnwrap) + ")");
            }
        }
    }

    /* JADX INFO: renamed from: ᛱᛲᛳᛴᛵᛶᛷᛸᤝᤞᲀᲁᲈᲇ, reason: contains not printable characters */
    public final void m3842() {
        Iterator it = this.f6304.iterator();
        while (it.hasNext()) {
            ((InterfaceC2706) it.next()).getClass();
        }
    }

    /* JADX INFO: renamed from: ᛱᛲᛳᛴᛵᛶᛷᛸᤝᤞᲀᲇᲁᲈ, reason: contains not printable characters */
    public final void m3843(Class cls, String str) {
        Iterator it = this.f6304.iterator();
        while (it.hasNext()) {
            if (!((InterfaceC2706) it.next()).mo3836(cls, str)) {
                int i = C2705.f8703;
                throw new C2705("Can't get this static field: " + cls.getTypeName() + "." + str);
            }
        }
    }

    /* JADX WARN: Code duplicated, block: B:66:0x016e  */
    /* JADX WARN: Code duplicated, block: B:83:0x019b  */
    /* JADX INFO: renamed from: ᛱᛲᛳᛴᛵᛶᛷᛸᤝᤞᲀᲇᲈᲁ, reason: contains not printable characters */
    public final void m3844(Object obj, String str, Object[] objArr) {
        Object[] objArr2;
        Object[] objArrCopyOfRange;
        Object[] objArrUnwrap = Primitive.unwrap(objArr);
        Iterator it = this.f6304.iterator();
        while (it.hasNext()) {
            if (!((InterfaceC2706) it.next()).mo3840(obj)) {
                int i = C2705.f8703;
                String typeName = obj.getClass().getTypeName();
                String strM4674 = C2705.m4674(objArrUnwrap);
                StringBuilder sb = new StringBuilder("Can't invoke this method: ");
                sb.append(typeName);
                sb.append(".");
                sb.append(str);
                sb.append("(");
                throw new C2705(AbstractC2844.m4786(sb, strM4674, ")"));
            }
        }
        if (str.equals("set") && objArrUnwrap.length == 2 && (obj instanceof Field)) {
            Field field = (Field) obj;
            String name = field.getName();
            Object obj2 = objArrUnwrap[1];
            Object[] objArr3 = AbstractC0016.f568;
            if (Modifier.isStatic(field.getModifiers())) {
                Class<?> declaringClass = field.getDeclaringClass();
                try {
                    m3847(obj2, declaringClass, name);
                } catch (C2705 unused) {
                    int i2 = C2705.f8703;
                    String typeName2 = declaringClass.getTypeName();
                    String strM4675 = C2705.m4674(new Object[]{obj2});
                    StringBuilder sb2 = new StringBuilder("Can't set this static field using reflection: ");
                    sb2.append(typeName2);
                    sb2.append(".");
                    sb2.append(name);
                    sb2.append(" (");
                    throw new C2705(AbstractC2844.m4786(sb2, strM4675, ")"));
                }
            } else {
                Object obj3 = objArrUnwrap[0];
                try {
                    m3846(obj3, name, obj2);
                } catch (C2705 unused2) {
                    int i3 = C2705.f8703;
                    String typeName3 = obj3.getClass().getTypeName();
                    String strM4676 = C2705.m4674(new Object[]{obj2});
                    StringBuilder sb3 = new StringBuilder("Can't set this field using reflection: ");
                    sb3.append(typeName3);
                    sb3.append(".");
                    sb3.append(name);
                    sb3.append(" (");
                    throw new C2705(AbstractC2844.m4786(sb3, strM4676, ")"));
                }
            }
        }
        if (str.equals("get") && objArrUnwrap.length == 1 && (obj instanceof Field)) {
            Field field2 = (Field) obj;
            String name2 = field2.getName();
            Object[] objArr4 = AbstractC0016.f568;
            if (Modifier.isStatic(field2.getModifiers())) {
                Class<?> declaringClass2 = field2.getDeclaringClass();
                try {
                    m3843(declaringClass2, name2);
                } catch (C2705 unused3) {
                    int i4 = C2705.f8703;
                    throw new C2705("Can't get this static field using reflection: " + declaringClass2.getTypeName() + "." + name2);
                }
            } else {
                Object obj4 = objArrUnwrap[0];
                try {
                    m3842();
                } catch (C2705 unused4) {
                    throw C2705.m4676(obj4, name2);
                }
            }
        }
        if ((obj instanceof Class) && str.equals("newInstance")) {
            Class cls = (Class) obj;
            Object[] objArr5 = new Object[0];
            try {
                m3841(cls, objArr5);
            } catch (C2705 unused5) {
                throw C2705.m4675(cls, objArr5);
            }
        } else if ((obj instanceof Constructor) && str.equals("newInstance")) {
            Class declaringClass3 = ((Constructor) obj).getDeclaringClass();
            if (objArrUnwrap.length == 1) {
                Object obj5 = objArrUnwrap[0];
                if (obj5 instanceof Object[]) {
                    objArr2 = (Object[]) obj5;
                } else {
                    objArr2 = objArrUnwrap;
                }
            } else {
                objArr2 = objArrUnwrap;
            }
            try {
                m3841(declaringClass3, objArr2);
            } catch (C2705 unused6) {
                throw C2705.m4675(declaringClass3, objArr2);
            }
        }
        if (str.equals("invoke") && objArrUnwrap.length != 0 && (obj instanceof Method)) {
            Method method = (Method) obj;
            String name3 = method.getName();
            if (objArrUnwrap.length == 2) {
                Object obj6 = objArrUnwrap[1];
                if (obj6 instanceof Object[]) {
                    objArrCopyOfRange = (Object[]) obj6;
                } else {
                    objArrCopyOfRange = Arrays.copyOfRange(objArrUnwrap, 1, objArrUnwrap.length);
                }
            } else {
                objArrCopyOfRange = Arrays.copyOfRange(objArrUnwrap, 1, objArrUnwrap.length);
            }
            Object[] objArr6 = AbstractC0016.f568;
            if (Modifier.isStatic(method.getModifiers())) {
                Class<?> declaringClass4 = method.getDeclaringClass();
                try {
                    m3845(declaringClass4, name3, objArrCopyOfRange);
                    return;
                } catch (C2705 unused7) {
                    int i5 = C2705.f8703;
                    String typeName4 = declaringClass4.getTypeName();
                    String strM4677 = C2705.m4674(objArrCopyOfRange);
                    StringBuilder sb4 = new StringBuilder("Can't invoke this static method using reflection: ");
                    sb4.append(typeName4);
                    sb4.append(".");
                    sb4.append(name3);
                    sb4.append("(");
                    throw new C2705(AbstractC2844.m4786(sb4, strM4677, ")"));
                }
            }
            Object obj7 = objArrUnwrap[0];
            try {
                m3844(obj7, name3, objArrCopyOfRange);
            } catch (C2705 unused8) {
                int i6 = C2705.f8703;
                String typeName5 = obj7.getClass().getTypeName();
                String strM4678 = C2705.m4674(objArrCopyOfRange);
                StringBuilder sb5 = new StringBuilder("Can't invoke this method using reflection: ");
                sb5.append(typeName5);
                sb5.append(".");
                sb5.append(name3);
                sb5.append("(");
                throw new C2705(AbstractC2844.m4786(sb5, strM4678, ")"));
            }
        }
    }

    /* JADX INFO: renamed from: ᛱᛲᛳᛴᛵᛶᛷᛸᤝᤞᲀᲈᲁᲇ, reason: contains not printable characters */
    public final void m3845(Class cls, String str, Object[] objArr) {
        Object[] objArrUnwrap = Primitive.unwrap(objArr);
        Iterator it = this.f6304.iterator();
        while (it.hasNext()) {
            ((InterfaceC2706) it.next()).getClass();
        }
        if (str.equals("getLength") && objArrUnwrap.length == 1 && cls.isAssignableFrom(Array.class)) {
            Object obj = objArrUnwrap[0];
            try {
                m3842();
            } catch (C2705 unused) {
                throw C2705.m4676(obj, "length");
            }
        }
    }

    /* JADX INFO: renamed from: ᛱᛲᛳᛴᛵᛶᛷᛸᤝᤞᲀᲈᲇᲁ, reason: contains not printable characters */
    public final void m3846(Object obj, String str, Object obj2) throws C2705 {
        Object objUnwrap = Primitive.unwrap(obj2);
        Iterator it = this.f6304.iterator();
        while (it.hasNext()) {
            if (!((InterfaceC2706) it.next()).mo3838(obj)) {
                int i = C2705.f8703;
                String typeName = obj.getClass().getTypeName();
                String strM4674 = C2705.m4674(new Object[]{objUnwrap});
                StringBuilder sb = new StringBuilder("Can't set this field: ");
                sb.append(typeName);
                sb.append(".");
                sb.append(str);
                sb.append(" (");
                throw new C2705(AbstractC2844.m4786(sb, strM4674, ")"));
            }
        }
    }

    /* JADX INFO: renamed from: ᛱᛲᛳᛴᛵᛶᛷᛸᤝᤞᲁᲀᲇᲈ, reason: contains not printable characters */
    public final void m3847(Object obj, Class cls, String str) throws C2705 {
        Object objUnwrap = Primitive.unwrap(obj);
        Iterator it = this.f6304.iterator();
        while (it.hasNext()) {
            if (!((InterfaceC2706) it.next()).mo3839(cls, str)) {
                int i = C2705.f8703;
                String typeName = cls.getTypeName();
                String strM4674 = C2705.m4674(new Object[]{objUnwrap});
                StringBuilder sb = new StringBuilder("Can't set this static field: ");
                sb.append(typeName);
                sb.append(".");
                sb.append(str);
                sb.append(" (");
                throw new C2705(AbstractC2844.m4786(sb, strM4674, ")"));
            }
        }
    }
}
